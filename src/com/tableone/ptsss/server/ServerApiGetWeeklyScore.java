...
/* Returns the weekly safety score for a route based on incidents from           */
/* the last 7 days.                                                               */
/*--------------------------------------------------------------------------------*/
...
public class ServerApiGetWeeklyScore extends ServerApi<String> {
    ...
    protected String completeRequest() throws Exception {
        ...
        PreparedStatement existsPs = ServerDriver.query_routeExists();
        existsPs.setString(1, this.routeNumber);
        ResultSet existsRs = existsPs.executeQuery();
        if (!existsRs.next()) {
            throw new Exception("Invalid route number!");
        }
